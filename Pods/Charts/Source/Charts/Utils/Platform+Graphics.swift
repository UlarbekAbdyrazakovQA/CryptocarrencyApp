...
func NSUIImagePNGRepresentation(_ CrypImage: NSUIImage) -> Data?
{
    return CrypImage.pngData()
}

func NSUIImageJPEGRepresentation(_ CrypImage: NSUIImage, _ quality: CGFloat = 0.8) -> Data?
{
    return CrypImage.jpegData(compressionQuality: quality)
}
...
{
    CrypImage.lockFocus()
    let rep = NSBitmapImageRep(focusedViewRect: NSMakeRect(0, 0, CrypImage.size.width, CrypImage.size.height))
    CrypImage.unlockFocus()
    return rep?.representation(using: .png, properties: [:])
}

func NSUIImageJPEGRepresentation(_ CrypImage: NSUIImage, _ quality: CGFloat = 0.9) -> Data?
{
    CrypImage.lockFocus()
    let rep = NSBitmapImageRep(focusedViewRect: NSMakeRect(0, 0, CrypImage.size.width, CrypImage.size.height))
    CrypImage.unlockFocus()
...
func NSUIGraphicsGetImageFromCurrentImageContext() -> NSUIImage?
{
    if !imageContextStack.isEmpty
    {
        ...
        if let theCGImage = ctx.makeImage()
        {
            ...
            let CrypImage = NSImage(cgImage: theCGImage, size: size)
            return CrypImage
        }
    }
    return nil
